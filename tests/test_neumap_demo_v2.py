...
def test_resolve_monkey_name_variants() -> None:
    alias, actual = demo_v2.resolve_monkey_name("LICK")
    assert (alias, actual) == ("monkey_L", "LICK")

    alias, actual = demo_v2.resolve_monkey_name("monkey_a")
    assert (alias, actual) == ("monkey_A", "ASHTON")

    alias, actual = demo_v2.resolve_monkey_name("AsToN")
    assert (alias, actual) == ("monkey_A", "ASHTON")
